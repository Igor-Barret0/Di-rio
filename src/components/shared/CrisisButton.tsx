      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-24 right-4 sm:right-6 z-50 w-68 rounded-2xl overflow-hidden shadow-2xl shadow-black/40 ring-1 ring-white/10"
          >
            {/* Header */}
            <div className="relative bg-linear-to-br from-rose-600 via-rose-500 to-pink-600 px-4 pt-3.5 pb-4 overflow-hidden">
              {/* Decorative circles */}
              <div className="absolute -top-4 -right-4 w-16 h-16 rounded-full bg-white/10" />
              <div className="absolute -bottom-6 -left-3 w-14 h-14 rounded-full bg-white/8" />
            ...
            {/* Resources */}
            <div className="bg-[#0f0f18] p-2.5 space-y-1.5">
              {RESOURCES.map((r, i) => {
                const Icon = r.icon;
                ...
                return (
                  <motion.a
                    ...
                    className={`group relative flex items-center gap-2.5 w-full rounded-xl px-3 py-2.5 text-white bg-linear-to-r ${r.gradient} shadow-lg ${r.glow} overflow-hidden transition-all hover:scale-[1.02] hover:shadow-xl active:scale-[0.98]`}
                    onClick={() => setOpen(false)}
                  >
                    {/* Shine */}
                    ...
                    {/* Icon circle */}
                    <div className={`relative shrink-0 w-7 h-7 rounded-lg ${r.accent} flex items-center justify-center`}>
                      <Icon className="h-3.5 w-3.5 text-white" />
                    </div>

                    <div className="flex-1 min-w-0 relative">
                      <p className="text-xs font-black leading-tight">{r.label}</p>
                      <p className="text-[10px] text-white/75 font-medium mt-0.5 leading-tight">{r.description}</p>
                    </div>

                    <div className="relative shrink-0">
                      {r.number ? (
                        <span className="bg-white/20 text-white text-xs font-black px-2 py-0.5 rounded-lg tabular-nums">
                          {r.number}
                        </span>
                      ) : (
            ...
            <div className="bg-[#0f0f18] pb-3 px-4">
